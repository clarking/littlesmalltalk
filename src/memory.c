...
/*
	in theory the objectTable should only be accessible to the memory
 	...
 	Never the less, for efficiency sake some of the macros can also be
...
object *mBlockAlloc(int memorySize) {
	...
	if (currentMemoryPosition + memorySize >= MemoryBlockSize) {
		...
		/*
			...
			MemoryBlockSize - currentMemoryPositon - 1  and putting it
			on the free list, but I think the savings is  potentially small
		*/
