...
object         hashEachElement(object dict,  int hash, int (*fun) (object));
...
/*
  ...
  given a message and a class to start looking in, find the
...
static boolean findMethod(object *methodClassLocation)
{
	...
	for (; methodClass != nilobj; methodClass = basicAt(methodClass, superClassInClass)) {
